...
def search_params(X_train,y_train,X_val, y_val):
    ...
    random_search = RandomizedSearchCV(estimator=xgb_model, param_distributions=param_dist,
                                       n_iter=10, scoring='neg_mean_absolute_percentage_error', cv=3,
...
if __name__ == '__main__':
    ...
    features = ['Origin','Destination','O Dig','D Dig',  # 起点与终点特征
                ...
                ] + [f'mean_TravelTime_before_{i}' for i in range(1,7)] + [f'std_TravelTime_before_{i}' for i in range(2,7)] # 时序特征
    target = ['Travel Time']
    ...
    for n in n_lst:
        ...
        # 读取训练数据
        DiDi_df = pd.read_csv(f"TimeFeature/test/VehicleData_n{n}.csv",encoding='utf-8')
        data_df = pd.read_csv(f"TimeFeature/test/VehicleData_n{n}_total.csv",encoding='utf-8')
        ...
        for date in date_lst:
            ...
            if date < 6:
                test_date1,test_date2,test_date3 = 2,4,6
            elif date < 13:
                test_date1,test_date2,test_date3 = 9,11,13
            else:
                test_date1,test_date2,test_date3 = 16,28,20
            ...
            DiDi_test_df = DiDi_df[(DiDi_df['Date']==test_date3)].reset_index(drop=True)

            # DiDi总的测试数据
            # DiDi_total_test_df = TOTAL_TEST_DF[TOTAL_TEST_DF['Date'].between(test_date1,test_date2)].reset_index(drop=True)

            DiDi_total_test_df = data_df[data_df['Date']==test_date3].reset_index(drop=True)
            ...
            DiDi_performance['Type'],DiDi_performance['Note'],DiDi_performance['Pro'],DiDi_performance['n'],DiDi_performance['date'] = 'DiDi','pure',DiDi_train_valid_df.shape[0]/data_df[data_df['Date']<=date].shape[0],n,date
            DiDi_total_performance['Type'],DiDi_total_performance['Note'],DiDi_total_performance['Pro'],DiDi_total_performance['n'],DiDi_total_performance['date'] = 'DiDi','total',DiDi_train_valid_df.shape[0]/data_df[data_df['Date']<=date].shape[0],n,date
            ...
            for Autonavi_pro in pro_lst:
                ...
                Autonavi_df = pd.read_csv(f"TimeFeature/test/VehicleData_n{n}_pro{Autonavi_pro}.csv",encoding='utf-8')

                # Autonavi_total_test_df = TOTAL_TEST_DF[TOTAL_TEST_DF['Date'].between(test_date1,test_date2)].reset_index(drop=True)

                Autonavi_total_test_df = data_df[data_df['Date']==test_date3].reset_index(drop=True)
                ...
                Autonavi_test_df = Autonavi_df[(Autonavi_df['Date']==test_date3)].reset_index(drop=True)
